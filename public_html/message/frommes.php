...
<tr>
	...
	<div class="boxLink">
	<font size='5' color="#FF0000">|　</font><a href="frommes.php"><font color="FF0000"><b>受信BOX</b></font></a>
	</div>
	<div class="boxLink">
	...
	<?php
		$recordto=mysql_query("SELECT * FROM $tablename WHERE (tomemberid='{$_SESSION[memberid]}')");//自分宛のメッセージ
	...
	while ($tableto =mysql_fetch_assoc($recordto)){

		$recordprofiles=mysql_query("SELECT * FROM profiles WHERE (memberid='{$tableto[frommemberid]}') ORDER BY `created` DESC");//送ってきた人のプロフィール
	...
	<table >
		<tr>
			...
			<?php
			$recordtype=mysql_query("SELECT * FROM $tablename WHERE (id='{$tableto[id]}')");
			$tabletype =mysql_fetch_assoc($recordtype);
			if($tabletype[type]==='new'){
				print("<font size='1' color='#3399ff'>未返信</font>");
			}elseif($tabletype[type]==='reply'){
				print("<font size='1' color='#cc33ff'>返信済</font>");
			}
			?>
			</td>
 			<td width="150"><font size='2'><b><?php print(htmlspecialchars($tableprofiles[handle])); ?></b></font></td>
 			<td width="200"><a href='message.php?id=<?php print($tableto[id]); print("&"); print("message=from");?>'><b><?php print(htmlspecialchars($tableto[title])); ?></b></a></td>
 			<td width="320"><font color='#707070'><b><?php print(mb_substr($tableto[message] ,0,14));?></b><?php print(" ..."); ?></font></td>
 			<td width="100"><b><font size='2'><?php print(htmlspecialchars($tableto[created])); ?></font></b></td>
