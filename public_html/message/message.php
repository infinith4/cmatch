...
<?php
session_start();
if(empty($_SESSION[memberid])||empty($_REQUEST['id'])){
...
$recordmes=mysql_query("SELECT * FROM $tablename WHERE (id='{$_REQUEST['id']}')");

$tablemes =mysql_fetch_assoc($recordmes);

//print("request:".$_REQUEST[message]);


if($_REQUEST[message]==='from'){

	$recordprofiles=mysql_query("SELECT * FROM profiles WHERE (memberid='{$tablemes[frommemberid]}')");


	$tableprofiles =mysql_fetch_assoc($recordprofiles);
}elseif($_REQUEST[message]==='to'){

	$recordprofiles=mysql_query("SELECT * FROM profiles WHERE (memberid='{$tablemes[tomemberid]}')");
...
<tr>
	...
	<td valign="top">
<div align="center">
	<div id="message-box" align="left" class="radius">

	<hr>

	<font size="3"><b><?php print($tablemes[title]); ?></b></font><br><br>
	
	<?php
	if($_REQUEST[message]==='from'){
	print("<div align='right'>");
		$_SESSION[replymemberid]=$tablemes[frommemberid];
	?>
	<a href="send.php?replyid=<?php print($_REQUEST[id]); ?>&type=reply">返信</a>
	</div>
	<?php
	}elseif($_REQUEST[message]==='to'){
		$_SESSION[replymemberid]='';
	}
	?>
	
	<?php print($tablemes[created]); ?>
	<hr>
	<?php
	if($_REQUEST[message]==='from'){
		print("FROM：");
	...
	?>
	<a href="../membersclass.php?membersid=<?php print($tableprofiles[memberid]);?>">
